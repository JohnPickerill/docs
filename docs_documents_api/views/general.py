...
def cascade_health(depth):
    if (depth < 0) or (depth > app.config.get("MAX_HEALTH_CASCADE")):
        app.logger.error("Cascade depth {} out of allowed range (0 - {})"
                                 .format(depth, app.config.get("MAX_HEALTH_CASCADE")))
        return Response(response=json.dumps({
    ...
    overall_status = 200
    if app.config.get("DEPENDENCIES") is not None:
        for dependency, value in app.config.get("DEPENDENCIES").items():
            ...
            #        app.logger.error("Unknown error occured during health cascade on request to database: {};"
            ...
            if depth > 0:
                ...
                try:
                    ...
                    app.logger.error("Connection Aborted during health cascade on attempt to connect to {};"
                                             " full error: {}".format(dependency, e))
                    ...
                    app.logger.error("Unknown error occured during health cascade on request to {};"
                                             " full error: {}".format(dependency, e))
                    service["status"] = "UNKNOWN"
                    overall_status = 500
                    service["status_code"] = None
...
@general.route("/about")
def _about():
    from docs_documents_api import __version__, __service_name__
    about= {'service': __service_name__,
            'version': __version__ }
